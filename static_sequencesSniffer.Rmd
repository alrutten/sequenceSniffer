...
fn <- "/home/anne/personal/pruittdata/Keiser_and_Pruitt_2014/BT data for Nick.csv"
...
groupingVars <-c("Singleton.Pair", "Behavioral.Types", "Treatment", "key")
```
...
findDuplicates <- function(somevector, min_length, ignoreAllEqual=FALSE, sep=" ") {
  ...
  while (!done) {
    ...
    if (ignoreAllEqual & nrow(duplicates)>0) {
      duplicates <- duplicates[rle(duplicates$ngrams[[1]])$lengths[[1]]<i,]
      if (length(duplicates) ==0) done=TRUE
    }
    if (!done) {
...
if (length(ids)>0) markedTable <- markedTable %>%
